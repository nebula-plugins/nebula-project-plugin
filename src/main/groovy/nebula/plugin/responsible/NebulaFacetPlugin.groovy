...
import nebula.project.internal.ConfigurationUtil
...
class NebulaFacetPlugin implements Plugin<Project> {
    ...
    @Inject
    NebulaFacetPlugin(ObjectFactory objectFactory) {
        objects = objectFactory
    }

    @Override
    void apply(Project project) {
        extension = container(project, FacetDefinition, new NamedDomainObjectFactory<FacetDefinition>() {
            @Override
        ...
        extension.all { FacetDefinition facet ->
            ...
            project.plugins.withType(JavaBasePlugin) {
                ...
                sourceSets.matching { SourceSet sourceSet -> sourceSet.name == facet.parentSourceSet.get() }.all { SourceSet parentSourceSet ->
                    ...
                    ConfigurationUtil.extendSourceSet(project, parentSourceSet, sourceSet)
                    ...
                    if (facet instanceof TestFacetDefinition) {
                        TaskProvider<Test> testTask = createTestTask(project, facet.testTaskName.get(), sourceSet)
     ...
     */
    static TaskProvider<Test> createTestTask(Project project, String testName, SourceSet sourceSet) {
    ...
    SourceSet createSourceSet(Project project, SourceSet parentSourceSet, FacetDefinition set) {
        ...
        sourceSets.create(set.name) { SourceSet sourceSet ->
            ...
            addParentSourceSetOutputs(project, compileClasspath, parentSourceSet, sourceSets)
            ...
            addParentSourceSetOutputs(project, runtimeClasspath, parentSourceSet, sourceSets)

            sourceSet.runtimeClasspath = project.objects.fileCollection().from(runtimeClasspath as Object[])
        }
    }

    private void addParentSourceSetOutputs(
            Project project,
        ...
        if (extendsFrom.name == IMPLEMENTATION_CONFIG_NAME) {
            addParentSourceSetOutputs(project, classpath, sourceSets.getByName(MAIN_SOURCE_SET_NAME), sourceSets)
        } else if (extendsFrom.name.toLowerCase().contains(IMPLEMENTATION_CONFIG_NAME)) {
            addParentSourceSetOutputs(project, classpath, sourceSets.getByName(extendsFrom.name.replaceAll(IMPLEMENTATION_CONFIG_NAME.capitalize(), '')), sourceSets)
        }
    }

    def <C> NamedContainerProperOrder<C> container(Project project, Class<C> type, NamedDomainObjectFactory<C> factory) {
    ...
    TestFacetDefinition createTestFacet(String name, Action<? super TestFacetDefinition> configureAction) {
        TestFacetDefinition facet = objects.newInstance(TestFacetDefinition, name)
    ...
    FacetDefinition createFacet(String name, Action<? super FacetDefinition> configureAction) {
        FacetDefinition facet = objects.newInstance(FacetDefinition, name)
        configureAction.execute(facet)
        extension.add(facet)
        return facet
    }
